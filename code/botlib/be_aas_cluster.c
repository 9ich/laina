...
void
AAS_ClearCluster(int clusternum)
{
	int i;
...
void
AAS_RemovePortalsClusterReference(int clusternum)
{
	int portalnum;
...
int
AAS_UpdatePortal(int areanum, int clusternum)
{
	int portalnum;
	aas_portal_t *portal;
...
int
AAS_FloodClusterAreas_r(int areanum, int clusternum)
{
	aas_area_t *area;
	aas_face_t *face;
	int facenum, i;
	...
	if(!nofaceflood)
		for(i = 0; i < area->numfaces; i++){
			...
			if(face->frontarea == areanum){
				if(face->backarea) if(!AAS_FloodClusterAreas_r(face->backarea, clusternum))
						return qfalse;
			}else if(face->frontarea) if(!AAS_FloodClusterAreas_r(face->frontarea, clusternum))
					return qfalse;
		}
...
int
AAS_FloodClusterAreasUsingReachabilities(int clusternum)
{
	int i, j, areanum;
...
void
AAS_NumberClusterPortals(int clusternum)
{
	int i, portalnum;
	aas_cluster_t *cluster;
	aas_portal_t *portal;
...
void
AAS_NumberClusterAreas(int clusternum)
{
	int i, portalnum;
	aas_cluster_t *cluster;
	aas_portal_t *portal;
...
void
AAS_ConnectedAreas_r(int *areanums, int numareas, int *connectedareas, int curarea)
{
...
int
AAS_GetAdjacentAreasWithLessPresenceTypes_r(int *areanums, int numareas, int curareanum)
{
...
int
AAS_CheckAreaForPossiblePortals(int areanum)
{
...
#if 0
void
AAS_FloodCluster_r(int areanum, int clusternum)
{
	int i, otherareanum;
	aas_face_t *face;
	aas_area_t *area;
...
void
AAS_RemoveTeleporterPortals(void)
{
	int i, j, areanum;
...
void
AAS_FloodClusterReachabilities(int clusternum)
{
	int i, j, areanum;
...
void
AAS_RemoveNotClusterClosingPortals(void)
{
...
void
AAS_CountForcedClusterPortals(void)
{
	int num, i;
...
void
AAS_SetViewPortalsAsClusterPortals(void)
{
	int i;
