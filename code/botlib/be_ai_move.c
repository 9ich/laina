...
int
BotFuzzyPointReachabilityArea(vec3_t origin)
{
...
int
BotReachabilityArea(vec3_t origin, int client)
{
...
int
BotOnMover(vec3_t origin, int entnum, aas_reachability_t *reach)
{
	int i, modelnum;
...
int
BotValidTravel(vec3_t origin, aas_reachability_t *reach, int travelflags)
{
...
float
VectorDistanceSquared(vec3_t p1, vec3_t p2)
{
	vec3_t dir;
...
int
BotAvoidSpots(vec3_t origin, aas_reachability_t *reach, bot_avoidspot_t *avoidspots, int numavoidspots)
{
...
void
BotAddAvoidSpot(int movestate, vec3_t origin, float radius, int type)
{
	bot_movestate_t *ms;
...
int
BotGetReachabilityToGoal(vec3_t origin, int areanum,
			 ...
			 struct bot_avoidspot_s *avoidspots, int numavoidspots, int *flags)
{
	...
	besttime = 0;
	bestreachnum = 0;
	for(reachnum = AAS_NextAreaReachability(areanum, 0)
	    ; reachnum;
...
int
BotAddToTarget(vec3_t start, vec3_t end, float maxdist, float *dist, vec3_t target)
{
	vec3_t dir;
	float curdist;
...
int
BotVisible(int ent, vec3_t eye, vec3_t target)
{
	bsp_trace_t trace;
...
int
BotPredictVisiblePosition(vec3_t origin, int areanum, bot_goal_t *goal, int travelflags, vec3_t target)
{
	aas_reachability_t reach;
...
void
MoverBottomCenter(aas_reachability_t *reach, vec3_t bottomcenter)
{
	int modelnum;
...
float
BotGapDistance(vec3_t origin, vec3_t hordir, int entnum)
{
	int dist;
	float startz;
	vec3_t start, end;
...
int
BotCheckBarrierJump(bot_movestate_t *ms, vec3_t dir, float speed)
{
...
int
BotSwimInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type)
{
	vec3_t normdir;
...
int
BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type)
{
...
int
BotMoveInDirection(int movestate, vec3_t dir, float speed, int type)
{
	bot_movestate_t *ms;
...
int
Intersection(vec2_t p1, vec2_t p2, vec2_t p3, vec2_t p4, vec2_t out)
{
...
void
BotCheckBlocked(bot_movestate_t *ms, vec3_t dir, int checkbottom, bot_moveresult_t *result)
{
	vec3_t mins, maxs, end, up = {0, 0, 1};
	bsp_trace_t trace;
...
int
BotAirControl(vec3_t origin, vec3_t velocity, vec3_t goal, vec3_t dir, float *speed)
{
	vec3_t org, vel;
	float dist;
	int i;
...
void
BotFuncBobStartEnd(aas_reachability_t *reach, vec3_t start, vec3_t end, vec3_t origin)
{
...
void
BotMoveToGoal(bot_moveresult_t *result, int movestate, bot_goal_t *goal, int travelflags)
{
