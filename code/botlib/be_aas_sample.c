...
void
AAS_PresenceTypeBoundingBox(int presencetype, vec3_t mins, vec3_t maxs)
{
	int index;
...
AAS_AllocAASLink(void)
{
	...
	if(aasworld.freelinks)
		aasworld.freelinks->prev_ent = nil;
	numaaslinks--;
	return link;
}

void
AAS_DeAllocAASLink(aas_link_t *link)
{
	if(aasworld.freelinks)
...
void
AAS_InitAASLinkedEntities(void)
{
	if(!aasworld.loaded)
		return;
...
void
AAS_FreeAASLinkedEntities(void)
{
	if(aasworld.arealinkedentities) FreeMemory(aasworld.arealinkedentities)
		;
...
int
AAS_PointReachabilityAreaIndex(vec3_t origin)
{
...
int
AAS_AreaPresenceType(int areanum)
{
	if(!aasworld.loaded)
		return 0;
	if(areanum <= 0 || areanum >= aasworld.numareas){
...
int
AAS_PointPresenceType(vec3_t point)
{
	int areanum;
...
vec_t
AAS_BoxOriginDistanceFromPlane(vec3_t normal, vec3_t mins, vec3_t maxs, int side)
{
	vec3_t v1, v2;
	int i;
...
aas_trace_t
AAS_TraceClientBBox(vec3_t start, vec3_t end, int presencetype,
		    int passent)
{
...
// recursive subdivision of the line by the BSP tree.
int
AAS_TraceAreas(vec3_t start, vec3_t end, int *areas, vec3_t *points, int maxareas)
{
	int side, nodenum, tmpplanenum;
	int numareas;
	float front, back, frac;
...
aas_face_t *
AAS_AreaGroundFace(int areanum, vec3_t point)
{
	int i, facenum;
	vec3_t up = {0, 0, 1};
	vec3_t normal;
...
void
AAS_FacePlane(int facenum, vec3_t normal, float *dist)
{
	aas_plane_t *plane;
...
int
AAS_BoxOnPlaneSide2(vec3_t absmins, vec3_t absmaxs, aas_plane_t *p)
{
	int i, sides;
	float dist1, dist2;
	vec3_t corners[2];
...
void
AAS_UnlinkFromAreas(aas_link_t *areas)
{
...
aas_link_t *
AAS_AASLinkEntity(vec3_t absmins, vec3_t absmaxs, int entnum)
{
	int side, nodenum;
...
aas_link_t *
AAS_LinkEntityClientBBox(vec3_t absmins, vec3_t absmaxs, int entnum, int presencetype)
{
	vec3_t mins, maxs;
...
int
AAS_BBoxAreas(vec3_t absmins, vec3_t absmaxs, int *areas, int maxareas)
{
...
int
AAS_AreaInfo(int areanum, aas_areainfo_t *info)
{
