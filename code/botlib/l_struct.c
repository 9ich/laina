...
fielddef_t *
FindField(fielddef_t *defs, char *name)
{
	int i;
...
ReadNumber(source_t *source, fielddef_t *fd, void *p)
{
	...
	//store the value
	if((fd->type & FT_TYPE) == FT_CHAR){
		if(fd->type & FT_UNSIGNED) *(uchar*)p = (uchar)
							intval;
		else *(char*)p = (char)intval;
	}else if((fd->type & FT_TYPE) == FT_INT){
		if(fd->type & FT_UNSIGNED) *(uint*)p = (uint)
						       intval;
...
int
ReadString(source_t *source, fielddef_t *fd, void *p)
{
	token_t token;
...
int
ReadStructure(source_t *source, structdef_t *def, char *structure)
{
	token_t token;
	fielddef_t *fd;
	void *p;
...
int
WriteIndent(FILE *fp, int indent)
{
	while(indent-- > 0)
		if(fprintf(fp, "\t") < 0)
			return qfalse;
...
int
WriteFloat(FILE *fp, float value)
{
	char buf[128];
	int l;
...
int
WriteStructWithIndent(FILE *fp, structdef_t *def, char *structure, int indent)
{
	int i, num;
	void *p;
	fielddef_t *fd;
