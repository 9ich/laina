...
int
AAS_BestReachableLinkArea(aas_link_t *areas)
{
	aas_link_t *link;
...
int
AAS_GetJumpPadInfo(int ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity)
{
	int modelnum, ent2;
...
int
AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs)
{
...
int
AAS_BestReachableArea(vec3_t origin, vec3_t mins, vec3_t maxs, vec3_t goalorigin)
{
	int areanum, i, j, k, l;
	aas_link_t *areas;
...
void
AAS_SetupReachabilityHeap(void)
{
	int i;
...
void
AAS_ShutDownReachabilityHeap(void)
{
...
void
AAS_FreeReachability(aas_lreachability_t *lreach)
{
...
int
AAS_AreaReachability(int areanum)
{
...
float
AAS_AreaGroundFaceArea(int areanum)
{
	int i;
	float total;
	aas_area_t *area;
...
void
AAS_FaceCenter(int facenum, vec3_t center)
{
	int i;
	float scale;
	aas_face_t *face;
...
float
AAS_MaxJumpHeight(float phys_jumpvel)
{
	float phys_gravity;
...
ushort
AAS_BarrierJumpTravelTime(void)
{
...
// from start to end
int
AAS_NearbySolidOrGap(vec3_t start, vec3_t end)
{
...
int
AAS_Reachability_Swim(int area1num, int area2num)
{
...
// heights
int
AAS_Reachability_EqualFloorHeight(int area1num, int area2num)
{
		...
		for(j = 0; j < area2->numfaces; j++){
			...
			for(edgenum1 = 0; edgenum1 < face1->numedges; edgenum1++)
				for(edgenum2 = 0; edgenum2 < face2->numedges; edgenum2++){
					...
					//if(AAS_PointAreaNum(end) != area2num)
					continue;
...
int
AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num, int area2num)
{
...
int
VectorBetweenVectors(vec3_t v, vec3_t v1, vec3_t v2)
{
	vec3_t dir1, dir2;
...
float
AAS_ClosestEdgePoints(vec3_t v1, vec3_t v2, vec3_t v3, vec3_t v4,
		      aas_plane_t *plane1, aas_plane_t *plane2,
		...
		if(dist > bestdist - 0.5 && dist < bestdist + 0.5){
			...
			if(dist1 > dist2){
				if(dist1 > VectorDistance(beststart1, beststart2)) VectorCopy(v1, beststart2)
					;
			}else if(dist2 > VectorDistance(beststart1, beststart2)) VectorCopy(v1, beststart1)
				;
			...
			if(dist1 > dist2){
				if(dist1 > VectorDistance(bestend1, bestend2)) VectorCopy(p1, bestend2)
					;
			}else if(dist2 > VectorDistance(bestend1, bestend2)) VectorCopy(p1, bestend1)
				;
		}else if(dist < bestdist){
			bestdist = dist;
		...
		if(dist > bestdist - 0.5 && dist < bestdist + 0.5){
			...
			if(dist1 > dist2){
				if(dist1 > VectorDistance(beststart1, beststart2)) VectorCopy(v2, beststart2)
					;
			}else if(dist2 > VectorDistance(beststart1, beststart2)) VectorCopy(v2, beststart1)
				;
			...
			if(dist1 > dist2){
				if(dist1 > VectorDistance(bestend1, bestend2)) VectorCopy(p2, bestend2)
					;
			}else if(dist2 > VectorDistance(bestend1, bestend2)) VectorCopy(p2, bestend1)
				;
		}else if(dist < bestdist){
			bestdist = dist;
		...
		if(dist > bestdist - 0.5 && dist < bestdist + 0.5){
			...
			if(dist1 > dist2){
				if(dist1 > VectorDistance(beststart1, beststart2)) VectorCopy(p3, beststart2)
					;
			}else if(dist2 > VectorDistance(beststart1, beststart2)) VectorCopy(p3, beststart1)
				;
			...
			if(dist1 > dist2){
				if(dist1 > VectorDistance(bestend1, bestend2)) VectorCopy(v3, bestend2)
					;
			}else if(dist2 > VectorDistance(bestend1, bestend2)) VectorCopy(v3, bestend1)
				;
		}else if(dist < bestdist){
			bestdist = dist;
		...
		if(dist > bestdist - 0.5 && dist < bestdist + 0.5){
			...
			if(dist1 > dist2){
				if(dist1 > VectorDistance(beststart1, beststart2)) VectorCopy(p4, beststart2)
					;
			}else if(dist2 > VectorDistance(beststart1, beststart2)) VectorCopy(p4, beststart1)
				;
			...
			if(dist1 > dist2){
				if(dist1 > VectorDistance(bestend1, bestend2)) VectorCopy(v4, bestend2)
					;
			}else if(dist2 > VectorDistance(bestend1, bestend2)) VectorCopy(v4, bestend1)
				;
		}else if(dist < bestdist){
			bestdist = dist;
...
int
AAS_Reachability_Jump(int area1num, int area2num)
{
...
int
AAS_Reachability_Ladder(int area1num, int area2num)
{
...
int
AAS_TravelFlagsForTeam(int ent)
{
	int notteam;
...
// targetname = "t2"
void
AAS_Reachability_Teleport(void)
{
...
// this is very game dependent.... :(
void
AAS_Reachability_Elevator(void)
{
	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)){
		...
		if(!strcmp(classname, "func_plat")){
			...
			if(!height) height = (maxs[2] - mins[2])
					     - lip;
...
aas_lreachability_t *
AAS_FindFaceReachabilities(vec3_t *facepoints, int numpoints, aas_plane_t *plane, int towardsface)
{
	int i, j, k, l;
...
void
AAS_Reachability_FuncBobbing(void)
{
...
int
AAS_Reachability_Grapple(int area1num, int area2num)
{
	...
	if(!AAS_AreaSwim(area1num)){
		...
		if(trace.startsolid)
			return qfalse;
		VectorCopy(trace.endpos, areastart);
	}else if(!(AAS_PointContents(start) & (CONTENTS_LAVA|CONTENTS_SLIME|CONTENTS_WATER)))
		return qfalse;
...
void
AAS_SetWeaponJumpAreaFlags(void)
{
	int ent, i;
...
int
AAS_Reachability_WeaponJump(int area1num, int area2num)
{
...
void
AAS_Reachability_WalkOffLedge(int areanum)
{
...
int
AAS_ContinueInitReachability(float time)
{
