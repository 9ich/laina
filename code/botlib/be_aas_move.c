...
AAS_WeaponJumpZVelocity(vec3_t origin, float radiusdamage)
{
	...
	AngleVectors(viewangles, forward, right, nil);
	...
	bsptrace = AAS_Trace(start, nil, nil, end, 1, CONTENTS_SOLID);
...
void
AAS_Accelerate(vec3_t velocity, float frametime, vec3_t wishdir, float wishspeed, float accel)
{
	// q2 style
	int i;
...
int
AAS_ClipToBBox(aas_trace_t *trace, vec3_t start, vec3_t end, int presencetype, vec3_t mins, vec3_t maxs)
{
	int i, j, side;
...
int
AAS_HorizontalVelocityForJump(float zvel, vec3_t start, vec3_t end, float *velocity)
{
