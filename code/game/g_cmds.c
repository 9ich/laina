...
*/
void
DeathmatchScoreboardMessage(ent_t *ent)
{
	char entry[1024];
	char string[1400];
	int stringlength;
...
static void
G_SayTo(ent_t *ent, ent_t *other, int mode, int color, const char *name, const char *message)
{
	if(!other)
		return;
	if(!other->inuse)
...
void
G_Say(ent_t *ent, ent_t *target, int mode, const char *chatText)
{
	int j;
	ent_t *other;
	int color;
