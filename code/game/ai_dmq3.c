...
BotCTFSeekGoals(bot_state_t *bs)
{
	...
	if(BotCTFCarryingFlag(bs)){
		...
		if(bs->ltgtype != LTG_RUSHBASE){
			...
			switch(BotTeam(bs)){
			case TEAM_RED:
				vecsub(bs->origin, ctf_blueflag.origin, dir);
				break;
			case TEAM_BLUE:
				vecsub(bs->origin, ctf_redflag.origin, dir);
				break;
			default:
				vecset(dir, 999, 999, 999);
				break;
			}
...
BotSetupForMovement(bot_state_t *bs)
{
	...
	veccopy(bs->cur_ps.origin, initmove.origin);
	veccopy(bs->cur_ps.velocity, initmove.velocity);
	...
	veccopy(bs->viewangles, initmove.viewangles);
...
BotCreateWayPoint(char *name, vec3_t origin, int areanum)
{
	...
	veccopy(origin, wp->goal.origin);
	veccopy(waypointmins, wp->goal.mins);
	veccopy(waypointmaxs, wp->goal.maxs);
...
BotGoCamp(bot_state_t *bs, bot_goal_t *goal)
{
	...
	if(camper > 0.99) bs->teamgoal_time = FloatTime()
					      + 99999;
...
BotRoamGoal(bot_state_t *bs, vec3_t goal)
{
	...
	for(i = 0; i < 10; i++){
		...
		if(len > 200){
			...
			vecscale(dir, len * trace.fraction - 40, dir);
...
BotAttackMove(bot_state_t *bs, int tfl)
{
	...
	if(attack_skill > 0.7) strafechange_time += crandom()
						    * 0.2;
...
BotSameTeam(bot_state_t *bs, int entnum)
{
	...
	if(gametype >= GT_TEAM)
		if(level.clients[bs->client].sess.team == level.clients[entnum].sess.team)
			return qtrue;
...
BotFindEnemy(bot_state_t *bs, int curenemy)
{
	...
	for(i = 0; i < maxclients && i < MAX_CLIENTS; i++){
		...
		if(curenemy >= 0) bs->enemysight_time = FloatTime()
							- 2;
...
BotAimAtEnemy(bot_state_t *bs)
{
	...
	vecscale(enemyvelocity, 1 / entinfo.update_time, enemyvelocity);
	...
	if(bs->enemyposition_time < FloatTime()){
		...
		veccopy(enemyvelocity, bs->enemyvelocity);
		veccopy(entinfo.origin, bs->enemyorigin);
	}
	...
	if(enemyvisible){
		veccopy(entinfo.origin, bestorigin);
		bestorigin[2] += 8;
		//get the start point shooting from
		//NOTE: the x and y projectile start offsets are ignored
		veccopy(bs->origin, start);
			...
			if(!(dist > 100 && veclensq(dir) < Square(32))){
				...
				if(aim_skill > 0.8 &&
				   ...
				   bs->cur_ps.weaponstate == WEAPON_READY){
					...
					vecscale(dir, 1 / entinfo.update_time, dir);
					veccopy(entinfo.origin, origin);
					origin[2] += 1;
					vecclear(cmdmove);
					...
					veccopy(move.endpos, bestorigin);
				...
				//if not that skilled do linear prediction
					...
					//best spot to aim at
					vecsadd(entinfo.origin, (dist / wi.speed) * speed, dir, bestorigin);
				}
			}
		}
					...
					if(veclensq(dir) < Square(60)){
						...
						if(veclensq(dir) > Square(100)){
							...
							if(trace.fraction >= 1)
								...
								veccopy(groundtarget, bestorigin);
						}
					}
				}
			}
		...
		veccopy(bs->lastenemyorigin, bestorigin);
		bestorigin[2] += 8;
	...
	if(enemyvisible){
		...
		veccopy(trace.endpos, bs->aimtarget);
	}else
		veccopy(bestorigin, bs->aimtarget);
	//get aim direction
	...
	if(bot_challenge.integer)
		...
		if(aim_accuracy > 0.9 && bs->enemysight_time < FloatTime() - 1){
			...
			veccopy(bs->ideal_viewangles, bs->viewangles);
...
BotCheckAttack(bot_state_t *bs)
{
	...
	//end point aiming at
	vecsadd(start, 1000, forward, end);
	//a little back to make sure not inside a very close enemy
	vecsadd(start, -12, forward, start);
...
BotFuncButtonActivateGoal(bot_state_t *bs, int bspent, bot_activategoal_t *activategoal)
{
	...
	if(health){
		...
		vecsadd(origin, -dist, movedir, goalorigin);
		veccopy(goalorigin, activategoal->target);
		...
		if(bsptrace.fraction >= 1.0 || bsptrace.ent == entitynum){
			...
			veccopy(bs->origin, activategoal->goal.origin);
			...
			if(i >= 0){
				veccopy(points[i], activategoal->goal.origin);
		...
		for(i = 0; i < numareas; i++)
			if(trap_AAS_AreaReachability(areas[i]))
				break;
		if(i < numareas){
			veccopy(origin, activategoal->goal.origin);
			activategoal->goal.areanum = areas[i];
...
BotFuncDoorActivateGoal(bot_state_t *bs, int bspent, bot_activategoal_t *activategoal)
{
	...
	vecadd(mins, maxs, origin);
	vecscale(origin, 0.5, origin);
	veccopy(origin, activategoal->target);
	activategoal->shoot = qtrue;
...
BotGoForActivateGoal(bot_state_t *bs, bot_activategoal_t *activategoal)
{
	...
	veccopy(activateinfo.origin, activategoal->origin);
...
BotRandomMove(bot_state_t *bs, bot_moveresult_t *moveresult)
{
	...
	veccopy(dir, moveresult->movedir);
}
...
BotAIBlocked(bot_state_t *bs, bot_moveresult_t *moveresult, int activate)
{
	...
	if(movetype != MOVE_CROUCH || !trap_BotMoveInDirection(bs->ms, hordir, 400, movetype))
		...
		if(!trap_BotMoveInDirection(bs->ms, sideward, 400, movetype)){
			...
			vecsadd(sideward, -1, hordir, sideward);
...
BotCheckEvents(bot_state_t *bs, entityState_t *state)
{
	...
	case EV_PLAYER_TELEPORT_IN: {
		veccopy(state->origin, lastteleport_origin);
...
BotGetAlternateRouteGoal(bot_state_t *bs, int base)
{
	...
	veccopy(altroutegoals[rnd].origin, goal->origin);
	vecset(goal->mins, -8, -8, -8);
	vecset(goal->maxs, 8, 8, 8);
	goal->entitynum = 0;
	goal->iteminfo = 0;
...
BotGoalForBSPEntity(char *classname, bot_goal_t *goal)
{
	...
	for(ent = trap_AAS_NextBSPEntity(0); ent; ent = trap_AAS_NextBSPEntity(ent)){
		...
		if(!strcmp(value, classname)){
			if(!trap_AAS_VectorForBSPEpairKey(ent, "origin", origin))
				return qfalse;
			veccopy(origin, goal->origin);
			veccopy(origin, start);
			start[2] -= 32;
			veccopy(origin, end);
			end[2] += 32;
			numareas = trap_AAS_TraceAreas(start, end, areas, nil, 10);
