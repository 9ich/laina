...
parseteaminfo(void)
{
	...
	if(nsortedteamplayers < 0 || nsortedteamplayers > TEAM_MAXOVERLAY){
		cgerrorf("parseteaminfo: nsortedteamplayers out of range (%d)",
			 nsortedteamplayers);
		return;
	}
	...
	for(i = 0; i < nsortedteamplayers; i++){
		...
		if(client < 0 || client >= MAX_CLIENTS){
			cgerrorf("parseteaminfo: bad client number: %d", client);
			return;
		}
...
configstringmodified(void)
{
	...
	if(num == CS_MUSIC)
		...
		Q_strncpyz(cgs.teamvotestr[num-CS_TEAMVOTE_STRING], str, sizeof(cgs.teamvotestr));
...
/*
=================
removechatescapechar
=================
*/
static void
removechatescapechar(char *text)
{
	int i, l;
