...
*/
void
drawchar(int x, int y, int width, int height, int ch)
{
	int row, col;
	float frow, fcol;
	float size;
   ...
   qboolean forceColor, qboolean shadow, int charWidth, int charHeight, int maxChars)
{
	...
	if(shadow){
		...
		while(*s && cnt < maxChars){
			...
			drawchar(xx + 2, y + 2, charWidth, charHeight, *s);
			cnt++;
			xx += charWidth;
			s++;
		}
...
drawpropstr(int x, int y, const char *str, int style, vec4_t color)
{
	...
	sizeScale = propsizescale(style);

	switch(style & UI_FORMATMASK){
	case UI_CENTER:
		width = propstrwidth(str) * sizeScale;
		x -= width / 2;
		break;
	...
	if(style & UI_DROPSHADOW){
		...
		drawcolor[3] = color[3];
		drawpropstr2(x+2, y+2, str, drawcolor, sizeScale, cgs.media.charsetProp);
	}
	...
	if(style & UI_INVERSE){
		...
		drawcolor[3] = color[3];
		drawpropstr2(x, y, str, drawcolor, sizeScale, cgs.media.charsetProp);
		return;
	}
